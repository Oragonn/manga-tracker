...
// Automatically attach the CSRF token to every same-origin state-changing
// fetch() call, so existing code doesn't need to be touched per call site.
(function attachCsrfToFetch() {
  ...
  window.fetch = function (input, init) {
    ...
    if (isSameOrigin && stateChangingMethods.has(method)) {
      init = init || {};
...
// title, tag name, or other value from an external/tracked source ends up
// in the DOM -- those aren't trusted just because they look like plain text.
...
function showNotification(message, type = 'added', duration = 4000) {
  // Escape first -- formatNotificationMessage wraps part of this in a <span>
  // itself, and the message often contains a series/tag name pulled straight
  // from an external tracked source, which isn't trustworthy HTML.
  message = formatNotificationMessage(escapeHtml(message));
